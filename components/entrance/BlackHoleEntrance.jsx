...
function CameraDrift({ metricState }) {
  ...
  useFrame(({ camera }) => {
    ...
    // Uses metricState.orbital and metricState.breathing for synchronized worldline
    const orbitalPhase = metricState.orbital; // 60s period
    const breathingPhase = metricState.breathing; // 45s period
    
    // Lissajous drift — subliminal amplitude, synchronized with metric evolution
...
function Scene({ phase, proximityDim = 0 }) {
  ...
  return (
    <>
      {/* Camera drift — subliminal Lissajous (metric-synchronized) */}
